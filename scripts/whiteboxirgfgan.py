...
from networks.gan import SpectNormDiscriminator, UnetGeneratorIRGF
...
class WhiteBoxGAN(pl.LightningModule):
  ...
  def __init__(
    ...
    # networks
    self.generator = UnetGeneratorIRGF()
  ...
  def forward(self, input_photo) -> torch.Tensor:
    ...
    output = self.guided_filter(input_photo, generator_img, r=1, eps=5e-2)
    return output
  ...
  def training_step(self, batch: Tuple[torch.Tensor, torch.Tensor], batch_idx, optimizer_idx):
    ...
    if optimizer_idx == 0:  # train generator
      ...
      ## removed the guided filter in the training step
      blur_fake = self.guided_filter(generator_img, generator_img, r=5, eps=2e-1)
      ...
      gray_fake, = self.colorshift(generator_img)
      ...
      input_superpixel = torch.from_numpy(
          simple_superpixel(generator_img.detach().permute((0, 2, 3, 1)).cpu().numpy(),
                            self.superpixel_fn)
      ...
      tv_loss = self.hparams.tv_weight * self.variation_loss(generator_img)
      ...
      # 2. gray for Textural Representation
      gray_fake, gray_cartoon = self.colorshift(generator_img, input_cartoon)
